...
import { saveStaffMember, deleteStaffMember } from "@/lib/actions/paye";

// Helpers moved outside or exported
...
export function PayeContent({ initialStaff }: { initialStaff: StaffMember[] }) {
    ...
    const [selectedEmployeeId, setSelectedEmployeeId] = useState(initialStaff[0]?.id || 0);
    ...
    const purgeFutureData = () => {
        ...
        const updatedStaff = staffMembers.map(emp => {
    ...
    const updateMonthData = async (empId: number, field: keyof ReturnType<typeof getMonthData>, value: number) => {
        let updatedEmp: StaffMember | null = null;
        ...
        if (updatedEmp) await saveStaffMember(updatedEmp);
    };
    ...
    const handleSave = async () => {
        if (editData) {
            ...
            await saveStaffMember(editData);
        }
        setIsEditing(false);
        setEditData(null);
    };
    ...
    const handleAddNew = async () => {
        ...
        await saveStaffMember(newEmployee);
    };
