...
export default function ProductionPage() {
    const [articles] = usePersistedState<Article[]>("bakery_articles", []);
    const [recipes, setRecipes] = usePersistedState<Recipe[]>("bakery_recipes", initialRecipes);
    ...
    const getFilteredArticles = (search: string) => {
        ...
        return articles.filter(a =>
            ...
            rawMaterialFamilies.includes(a.subFamilyId.substring(0, 4)) // Adjusting for actual subFamilyId prefixes
        );
    };

    // Helper: Calculate Real-time Recipe Costs
    ...
    const handleAddIngredient = (article: Article) => {
        if (!editData) return;
        const newIng = {
                        ...
                        {!selectedRecipe ? (
                            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 bg-slate-50/30">
                                ...
                                <button
                                    ...
                                    className="mt-4 px-6 py-2 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 transition-all shadow-md"
                                >
                                                    ...
                                                    <table className="w-full border-collapse">
                                                        ...
                                                        <tbody className="divide-y divide-slate-100">
                                                            {(() => {
                                                                ...
                                                                // 1. Calculate normalization to 'g' or base weight for % calculations
                                                                // Assuming quantity is in production unit (as defined in article)
                                                                // We'll need to fetch article data to get base price/unit info

                                                                const ingsWithCalculations = currentIngs.map(ing => {
                                                                    const article = articles.find(a => a.id === ing.articleId);
                                                                    // Logic: PU is article.lastPivotPrice converted to production unit
                                                                    const prodPrice = article ? (article.lastPivotPrice || 0) / (article.coeffProd || 1) : 0;
                                                                    const cost = ing.quantity * prodPrice * multiplier;
                                                                    // Weight in 'g' or base: assuming quantity is in production unit
                                                                    // If unit is 'g', weight = quantity. If unit is 'cl', we might need more logic but simplified for now.
                                                                                ...
                                                                                {isEditing && editData ? (
                                                                                    <div className="relative">
                                                                                        <input
                                                                                            ...
                                                                                            placeholder="Rechercher matière première..."
                                                                                            onKeyDown={(e) => {
                                                                ...
                                                                <>
                                                                    ...
                                                                    <div className="flex justify-between items-center p-4 bg-red-50 rounded-xl border border-red-100">
                                                                        ...
                                                                        <span className="font-bold text-red-700">
                                                                            +{(totals.totalCost * (selectedRecipe.costing.lossRate || 0) / 100).toFixed(2)} Dh
                                                                        </span>
                                                                    </div>
                                                                </>
                                                            );
